...
fun RYGeckoView(
    ...
    if (tonalElevation.value.toInt() == 0) {
        backgroundColor = if (isSystemInDarkTheme()) android.graphics.Color.parseColor("#202124") else android.graphics.Color.parseColor("#F4F0E6")
    }
    ...
    AndroidView(
        ...
        update = {
            it.apply {
                ...
                val formattedHtml = WebViewHtml.HTML.format(
                        WebViewStyle.get(
                            fontSize = fontSize,
                            fontPath = fontPath,
                ...
                geckoView.session?.loadUri("data:text/html;charset=utf-8," + android.net.Uri.encode(formattedHtml))
            }
        },
    )
}
